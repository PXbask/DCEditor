        ...
        private void UpdateLayerRefreshHierarchy(int count)
        {
            ...
            if (count > childCount)
            {
                for (int i = childCount; i < count; i++)
                {
                    ...
                    newObj.transform.position = new Vector3(pos.x, (2 * i + 1) * (extents.y+1), pos.z);

                    DCDominoCard card = Instantiate(dominoPrefab, newObj.transform);

                    DCLayer lr = newObj.AddComponent<DCLayer>();
                    lr.Init(i);
                    MirrorChildren mirror = newObj.AddComponent<MirrorChildren>();
                }
            }
        }
        #endregion
